...
void recgraph(int64_t *rumerbfs, int64_t nconf,int64_t posj,int64_t nleft,int64_t countidx,int64_t ms,int64_t n){
    int ifenter = 0;
    if(posj != 2*nleft){
        for(int i = posj; i < 2*nleft-1; i = i-1){
            ifenter = 1;
            int nzeros = 0;
            int nones = 0;
            for(int k = 0; k < i; k++){
            ...
            if(nleft > 0 and countidx+1 <= nconf){
                ...
                countidx+=1
                    if(countidx+1 <= nconf){
                        for(int k = 0; k < n; k++)
                            rumerbfs[countidx,k]=rumerbfs[countidx-1,k];
                        rumerbfs[countidx,i-1]=0;
                        nleft += 1;
                    }
            }
...
#+name: genBFspython
#+begin_src python
def recgraph(rumerbfs,nconf,posj,nleft,countidx,ms,n):
    ifenter = 0
    if posj != 2*nleft:
